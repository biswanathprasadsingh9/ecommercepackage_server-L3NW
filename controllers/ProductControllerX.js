...
const productsearch = async (req,res) => {
  ...
  console.log(req.body.search_sortby)
        ...
        query_paginaton.exec(async function (errs, data2) {
          ...
          res.json({
            ...
            other_attributes:{ //---config attribute
              ...
              '630829c5a60ff047d08da33c':await query_paginaton.distinct("myattributes.630829c5a60ff047d08da33c"),
              '630829d9a60ff047d08da33d':await query_paginaton.distinct("myattributes.630829d9a60ff047d08da33d"),
  ...
  // var searchData=[]
  //
  // var objectkeys=Object.keys(getbody);
  // objectkeys.forEach((item, i) => {
  //   if(getbody[item].length>0){
  //     searchData.push({fieldName:'myattributes.'+item,value:getbody[item]})
  //   }
  // });
  //
  ...
  //
  //
  // var query = Product.find().select({'updatedAt':0});;
  //
  // // var filters = [
  // //     {fieldName: "category", value: ['Car','Truck']},
  // //     {fieldName: "myattributes.Group", value: ['A']},
  ...
  if(attributedata.length===1){
      attributedata[0].values.forEach((item0, i1) => {
          ...
          var dts={
            ...
            images:[],
            videos:[],
            sku:parent.sku+'-'+item0.toLowerCase(),
            name:parent.name+' '+item0,
            stock:0,
            pricemain:[0],
            pricedisplay:0,
            pricetemp:0,
            totalconfigattribute:item0,
            [attributedata[0]._id]:item0,
          }
          datas.push(dts);
      });
  }
    ...
    attributedata[0].values.forEach((item0, i0) => {
      attributedata[1].values.forEach((item1, i1) => {
          var dts={
            ...
            images:[],
            videos:[],
            sku:parent.sku+'-'+item0.toLowerCase()+'-'+item1.toLowerCase(),
            name:parent.name+' '+item0+' '+item1,
            stock:0,
            pricemain:[0],
            pricedisplay:0,
            pricetemp:0,
      ...
      attributedata[1].values.forEach((item1, i1) => {
        attributedata[2].values.forEach((item2, i2) => {
          ...
          var dts={
            ...
            images:[],
            videos:[],
            sku:parent.sku+'-'+item0.toLowerCase()+'-'+item1.toLowerCase()+'-'+item2.toLowerCase(),
            name:parent.name+' '+item0+' '+item1+' '+item2,
            stock:0,
            pricemain:[0],
            pricedisplay:0,
            pricetemp:0,
        ...
        attributedata[2].values.forEach((item2, i2) => {
          attributedata[3].values.forEach((item3, i3) => {
            ...
            var dts={
              ...
              images:[],
              videos:[],
              sku:parent.sku+'-'+item0.toLowerCase()+'-'+item1.toLowerCase()+'-'+item2.toLowerCase()+'-'+item3.toLowerCase(),
              name:parent.name+' '+item0+' '+item1+' '+item2+' '+item3,
              stock:0,
              pricemain:[0],
              pricedisplay:0,
              pricetemp:0,
...
const create2config = (req,res) => {
  ...
  const update1 = { step:'step3config',myattributes };


  Product.findOneAndUpdate({_id:req.body._id},update1)
  .then(parentdata=>{
